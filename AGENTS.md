# AGENTS.md — database-utils

liquibase-shell + DB ops. Liquibase targets come from trl-infra .infra-config.yaml `liquibase_targets` (service, port-forward cfg, changelog location). Liquibase owns schema; some apps deploy Ecto-only — run liquibase-shell after changelogs land (e.g. tobornalp).
